...
const QUESTION_KEYS = ["district", "sector", "isProjectHolder"] as const;
const QUESTION_ICONS: React.ElementType[][] = [
...
export default function EligibilitePage() {
  ...
  const t = useTranslations("candidaterEligibilite");
  ...
  const questions = (t.raw("questions") as {
    ...
    options: q.options.map(
      (o, j): EligibilityOption => ({ ...o, icon: QUESTION_ICONS[i][j] })
    ),
  }));
  ...
  const currentKey = QUESTION_KEYS[step] as keyof EligibilityAnswer;
          ...
          {step < 3 ? (
            ...
            >
              ...
              <Link
                ...
                className="rounded-xl bg-teal px-8 py-4 font-['Space_Grotesk'] font-bold text-on-void text-lg transition hover:scale-[1.02] hover:shadow-lg hover:shadow-teal/20 active:scale-[0.98]"
              >
                {t("start")}
              </Link>
            </motion.div>
