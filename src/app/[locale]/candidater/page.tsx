...
const BENEFIT_ICONS = [Target, Handshake, TrendingUp, Users];
const locations = ["MANAKARA", "VOHIPENO", "FITOVINANY"];

export default function CandidaterPage() {
  const t = useTranslations("candidaterLanding");

  const benefits = (t.raw("benefits") as { title: string; description: string }[]).map(
    (b, i) => ({ ...b, icon: BENEFIT_ICONS[i] })
  );
        ...
        <section className="flex-1 px-6 py-12 lg:py-8 flex flex-col justify-center max-w-3xl mx-auto lg:mx-0 lg:max-w-none">
          ...
          >
            ...
            <h1 className="font-['Space_Grotesk'] text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-text-primary leading-[1.05]">
              {t("title1")}{" "}
              <span className="text-teal block mt-2">{t("title2")}</span>
            </h1>
          ...
          >
            {locations.map((loc) => (
              <span
                key={loc}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-green text-on-void text-xs font-bold tracking-wider"
              >
